...
const DialogEdit: React.FC<DialogEditProps> = (props) => {
  ...
  return (
    <Modal show={props.show} size="md" popup onClose={props.onClose} className="z-1" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', minWidth: '50%' }}>
      <Modal.Header>
        <div className="text-xl font-medium text-gray-900 dark:text-white">Category Edit</div>
      </Modal.Header>
      <Modal.Body>
        <Formik initialValues={{ id: props.category.id, name: props.category.name }} validationSchema={UpdateCategorySchema} onSubmit={handleSubmit}>
              ...
              <div className="space-y-6">
                <div className="mb-2 block">
                  ...
                  <Field
                    ...
                    />
                  {errors.name && touched.name && <span className="text-red-500 ml-1 mt-1 flex items-center text-xs font-medium tracking-wide">{errors.name}</span>}
                </div>
                <div className="mb-4.5 flex justify-center">
                  <button type="submit" className="w-full cursor-pointer rounded-lg border border-primary bg-primary p-4 text-white transition hover:bg-opacity-90">Update</button>
                </div>
              </div>
            </Form>
          )}
