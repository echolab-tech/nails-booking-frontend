...
import { assistantDelete, getAssistantShow, getIsDeleted, getListAssistant } from '@/services/assistants.service';
...
const TeamList = () => {
    ...
    const handleButtonClick = async (customerId: number) => {
      try {
        router.push(`/assistants/edit/${customerId}`);
      } catch (error) {
    ...
    const handleButtonDelete = async (
      ...
      try {
        ...
        if (isCheckDeleted.data[0].isDeleted == 0) {
          ...
          if (userConfirmed) {
            await assistantDelete(customerId);
            updateCategoryList();
            toast.success("Delete Success !!!");
          }
        } else if (isCheckDeleted.data[0].isDeleted == 1) {
          const userConfirmed = window.confirm(
            `When you delete this assistant, all booking data of this assistant will also be deleted, are you sure ?`,
          );
          if (userConfirmed) {
            await assistantDelete(customerId);
            updateCategoryList();
                    ...
                    <div className="flex items-center space-x-3.5">
                      ...
                      <button className="hover:text-primary">
                        <svg
                          ...
                          xmlns="http://www.w3.org/2000/svg"
                          onClick={() =>
                            handleButtonDelete(teamItem?.id, teamItem.name)
                          }
                        >
                          <path
                      ...
                      <button className="hover:text-primary">
                        <svg
                          ...
                          stroke="currentColor"
                          className="h-6 w-6"
                          onClick={() => handleButtonClick(teamItem?.id)}
                        >
                          <path
                            strokeLinecap="round"
