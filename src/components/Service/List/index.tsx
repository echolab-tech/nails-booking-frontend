...
const ServiceList = () => {
    ...
    const currentServiceData = serviceData.slice(startIndex, endIndex);
        ...
        <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
            <div className="max-w-full overflow-x-auto">
                <table className="w-full table-auto">
                    ...
                    <tbody>
                        {currentServiceData.map((serviceItem, index) => (
                            <tr key={index}>
                                ...
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="font-medium text-black dark:text-white">
                                        {serviceItem.serviceOptions.map((option, index) => (
                                            <span key={index}>{formatPrice(option.price)}</span>
                                        ))}
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                ...
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p>
                                        {serviceItem.serviceCombos && serviceItem.serviceCombos.length > 0 ? "Combo" : "simple"}
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
            ...
            <div className="flex justify-center mt-4">
                ...
                <button
                    onClick={() => handlePageChange(currentPage + 1)}
                    disabled={endIndex >= serviceData.length}
                >
                    Next
                </button>
            </div>
