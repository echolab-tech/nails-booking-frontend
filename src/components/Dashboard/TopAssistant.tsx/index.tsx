...
const TopAssistantsList = () => {
  const [topAssistantsData, setTopAssistantsData] = useState<any[]>([]);

  useEffect(() => {
    fetchTopAssistants();
  }, []);

  const fetchTopAssistants = async () => {
    try {
  ...
  return (
    <div className="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:col-span-6">
      <div className="max-w-full overflow-x-auto">
        <p className="pb-4 text-xl font-semibold text-black">Top member</p>
        <table className="w-full table-auto">
          ...
          <tbody>
            {topAssistantsData.map((assistant, index) => (
              <tr key={index}>
                ...
                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                  {assistant.currentMonthBookings}
                </td>
                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                  {assistant.lastMonthBookings}
                </td>
              </tr>
            ))}
          </tbody>
...
export default TopAssistantsList;
